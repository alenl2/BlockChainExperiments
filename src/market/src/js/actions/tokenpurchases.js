...
import {updateTokensBalance, updateEtherBalance} from './accounts'
...
export const fulfill = (tokenPurchaseAddress, seller) => async dispatch => {
  ...
  try {
    ...
    await erc20.approve(tokenPurchase.address, toTokens(amount, decimals), { from: seller })
    dispatch(FetchingActions.start('Claiming your ether to the token purchase contract'))
    await tokenPurchase.claim({ from: seller})
    dispatch(updateEtherBalance(seller))
    dispatch(updateTokensBalance(seller, erc20Address))
...
export const create = (erc20Address, purchaser, amount, pricePerTokenInEther) => async dispatch => {
  dispatch(FetchingActions.start('Creating your token purchase contract'))
  try {
    const erc20 = await ERC20.at(erc20Address)
    const decimals = await erc20.decimals()
    const factory = await TokenPurchaseFactory.deployed()
    const transaction = await factory.createTokenPurchase(erc20Address, toTokens(amount, decimals), { from: purchaser })
    ...
    await tokenSale.sendTransaction({ from: purchaser, value: toWei(price) })
    dispatch(updateEtherBalance(purchaser))
...
export const refund = (tokenPurchaseAddress, purchaser) => async dispatch => {
  dispatch(FetchingActions.start('Refunding your token purchase contract'))
  try {
    const tokenPurchase = await TokenPurchase.at(tokenPurchaseAddress)
    const erc20Address = await tokenPurchase.token()
    await tokenPurchase.refund({ from: purchaser})
    dispatch(updateEtherBalance(purchaser))
    dispatch(updateTokensBalance(purchaser, erc20Address))
    dispatch(TokenPurchaseActions.load(tokenPurchase))
  } catch (error) {
    dispatch(ErrorActions.show(error))
  }
...
export const find = (tokenPurchaseAddress) => async dispatch => {
  try {
...
export const findAll = () => async dispatch => {
  try {
    const factory = await TokenPurchaseFactory.deployed()
    const events = factory.TokenPurchaseCreated({}, { fromBlock: 0, toBlock: 'latest' });
    events.watch(function(error, result) {
      if(error) ErrorActions.show(error)
