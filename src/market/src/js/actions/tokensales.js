...
import {updateEtherBalance, updateTokensBalance} from './accounts'
...
export const fulfill = (tokenSaleAddress, purchaser) => async dispatch => {
...
export const refund = (tokenSaleAddress, seller) => async dispatch => {
  dispatch(FetchingActions.start('Refunding your token sale contract'))
  try {
    const tokenSale = await TokenSale.at(tokenSaleAddress)
    const erc20Address = await tokenSale.token()
    await tokenSale.refund({ from: seller})
...
export const create = (erc20Address, seller, amount, pricePerTokenInEther) => async dispatch => {
  ...
  try {
    ...
    const transaction = await factory.createTokenSale(erc20Address, toWei(price), { from: seller})
    ...
    const erc20 = await ERC20.at(erc20Address)
    const decimals = await erc20.decimals()
    await erc20.transfer(tokenSaleAddress, toTokens(amount, decimals), { from: seller})
    dispatch(updateEtherBalance(seller))
    dispatch(updateTokensBalance(seller, erc20Address))
...
export const find = (tokenSaleAddress) => async dispatch => {
  try {
...
export const findAll = () => async dispatch => {
    try {
      const factory = await TokenSaleFactory.deployed()
      const events = factory.TokenSaleCreated({}, { fromBlock: 0, toBlock: 'latest' });
      events.watch(function(error, result) {
        if(error) ErrorActions.show(error)
